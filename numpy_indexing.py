...
def ex03_ten_percent_raise(salaries):
    ...
    salaries_increase = salaries * 1.1
    return salaries_increase
...
def ex06_weather_stats(temps):
    ...
    mean_temp = np.round(np.mean(temps), 1)
    max_temp = temps.max()
    ar_max_temp = temps.argmax()
    return mean_temp, max_temp, ar_max_temp


def ex07_warm_days(temps):
    """Return only the temperatures strictly above 15."""
    high_temp = temps[temps > 15]
    print(high_temp)
    return high_temp


def ex08_count_warm_days(temps):
    """Return HOW MANY temperatures are strictly above 15
    (an int)."""
    high_temp = temps > 15
    count = sum(high_temp)
    result = int(count)
...
def ex09_replace_outliers(values):
    ...
    new_values = np.where(values > 100, 100, values)
...
def ex10_median_vs_mean(salaries):
    ...
    mean_result = salaries.mean()
    media_result = np.median(salaries)
    print(mean_result)
    print(media_result)
    return mean_result, media_result
...
def ex12_cosine(a, b):
    ...
    return round(a @ b / (np.linalg.norm(a) * (np.linalg.norm(b))), 3)
...
def ex15_most_common(words):
    ...
    arg_max_words = unique_words.argmax()
    arg_max_count = counts.argmax()

    print(unique_words)
    print(counts)
    print(
        f"The most common word is: {unique_words[arg_max_count]} with {arg_max_words} times."
    )

    return unique_words[arg_max_count]
...
def ex19_stack_rows(a, b):
    ...
    stack_row = np.concatenate([a, b], axis=0)
    print(stack_row)
    return stack_row
...
def ex20_add_column(a, col):
    ...
    col_colum = col.reshape(-1, 1)
    col_add = np.hstack([a, col_colum])
    print(col_add)
...
def _run_tests():
    ...
    checks = [
        ...
        (
            "ex03",
            lambda: np.allclose(
                ex03_ten_percent_raise(np.array([100.0, 200.0])), [110.0, 220.0]
            ),
        ),
        ...
        (
            ...
            lambda: np.array_equal(
                ex09_replace_outliers(np.array([5, 150, 99, 101])), [5, 100, 99, 100]
            ),
        ),
        ...
        (
            "ex11",
            lambda: np.isclose(
                ex11_shopping_total(np.array([2, 1, 3]), np.array([4.5, 12.0, 2.25])),
                27.75,
            ),
        ),
        (
            "ex12",
            lambda: ex12_cosine(np.array([0.2, 0.9, 0.1]), np.array([0.3, 0.8, 0.2]))
            == 0.983,
        ),
        (
            "ex13",
            lambda: np.allclose(ex13_normalise_image(img), img / 255.0)
            and ex13_normalise_image(img).dtype == np.float64,
        ),
        ("ex14", lambda: np.array_equal(ex14_flip_vertically(img), img[::-1])),
        ("ex15", lambda: ex15_most_common(words) == "python"),
        (
            "ex16",
            lambda: np.array_equal(ex16_safe_copy(orig), [999, 2, 3])
            and np.array_equal(orig, [1, 2, 3]),
        ),
        (
            "ex17",
            lambda: np.array_equal(
                ex17_brighten(img), [[100, 228, 255], [164, 255, 132]]
            )
            and ex17_brighten(img).dtype == np.uint8,
        ),
        (
            "ex18",
            lambda: np.array_equal(ex18_top_n(np.array([4, 9, 1, 7, 3]), 3), [9, 7, 4]),
        ),
        (
            "ex19",
            lambda: np.array_equal(
                ex19_stack_rows(np.array([[1, 2], [3, 4]]), np.array([[5, 6]])),
                [[1, 2], [3, 4], [5, 6]],
            ),
        ),
        (
            "ex20",
            lambda: np.array_equal(
                ex20_add_column(np.array([[1, 2], [3, 4]]), np.array([9, 9])),
                [[1, 2, 9], [3, 4, 9]],
            ),
        ),
        (
            "ex21",
            lambda: np.array_equal(
                ex21_broadcast_row(np.array([[1, 2], [3, 4]]), np.array([[5, 6]])),
                [[6, 8], [8, 10]],
            ),
        ),
        (
            "ex22",
            lambda: np.allclose(
                ex22_centre_columns(np.array([[1, 2], [3, 4]])), [[-1, -1], [1, 1]]
            ),
        ),
        (
            "ex23",
            lambda: np.array_equal(
                ex23_view_trap(np.array([[1, 2], [3, 4]])), [[99, 2], [3, 4]]
            ),
        ),
    ]
    passed = 0
